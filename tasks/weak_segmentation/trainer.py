...
class Trainer(object):
    def __init__(self, settings: Option, model: nn.Module, recorder=None):
        ...
        self.evaluator = pc_processor.metrics.IOUEval(
            n_classes=self.settings.n_classes, device=torch.device("cuda"),
            ignore=self.ignore_class)
        self.evaluator.reset()

    def _loadCheckpoint(self, model, only_encoder=False, pretrain_path=None):

        assert pretrain_path is None or self.settings.checkpoint is None, "cannot use pretrained weight and checkpoint at the same time"
    ...
    def _initDataloader(self):
        if self.settings.dataset == "SemanticKitti":
            ...
            train_salsa_loader = pc_processor.dataset.semantic_poss.wss_sem_poss_loader.SalsaNextLoader(
                dataset=trainset,
                config=self.settings.config,
                is_train=True,
                is_weak_label=self.settings.weak_label,
            ...
            val_salsa_loader = pc_processor.dataset.semantic_poss.wss_sem_poss_loader.SalsaNextLoader(
                dataset=valset,
                config=self.settings.config,
                is_train=False,
                is_weak_label=self.settings.weak_label,
            ...
            train_salsa_loader = pc_processor.dataset.nuScenes.wss_nuscenes_loader.SalsaNextLoader(
                dataset=trainset,
                config=self.settings.config,
                is_train=True,
                is_weak_label=self.settings.weak_label,
            ...
            val_salsa_loader = pc_processor.dataset.nuScenes.wss_nuscenes_loader.SalsaNextLoader(
                dataset=valset,
                config=self.settings.config,
                is_train=False,
                is_weak_label=self.settings.weak_label,
        ...
        # config
        cls_counts = np.asarray([i for i in self.settings.config['cls_counts'].values()])
    ...
    def _initCriterion(self):
        ...
        criterion["lovasz"] = pc_processor.loss.Lovasz_softmax(ignore=self.settings.ignore_cls,
                                                               per_image=False, softmax=False)
    ...
    def _combineTensorboradImages(self,
                                  argmax: torch.Tensor,
        ...
        color_weak_label = (np.array(self.colormap[weak_label])).astype(np.uint8)  # h, w, 3, [0,255]

        if is_weak:
            # dilate
            color_weak_label = cv2.dilate(color_weak_label, np.ones((5, 5), np.uint8),
                                          iterations=1)  # h, w, 3, [0,255]

        color_weak_label = torch.from_numpy(color_weak_label).permute(2, 0, 1)  # 3, h, w

        # concat

        result = torch.cat([color_weak_label / 255, (color_argmax.permute(2, 0, 1)) / 255,
                            (color_full_label.permute(2, 0, 1)) / 255, (error_map.permute(2, 0, 1)) / 255],
                           1)  # 3, h*4, w
    ...
    def entropy_based_selection(self,
        ...
        entropy = -torch.sum(output * torch.log(output + 1e-10), dim=1)  # b, h, w  [0, ]
        ...
        for b in range(bs):
            ...
            for cls in unique_class:
                ...
                index_of_pseudo_labels = torch.multinomial(weight_c.reshape(-1), select_num, replacement=False)

                pseudo_labels_mask_c = torch.zeros_like(weight_c).reshape(-1).cuda().bool()
    ...
    def run(self, epoch, mode="Train"):
        ...
        return_feat = True \
            if epoch >= self.settings.contrast_warmup and mode == "Train" else False
        entropy_selection = self.settings.entropy_selection \
            if epoch >= self.settings.contrast_warmup and mode == "Train" else False
        ...
        for i, (
                input_feature, train_label, eval_label, _, tar_frame, seq_id, frame_id,
                unproj_full_labels, unproj_weak_labels, uproj_x_idx, uproj_y_idx
            ...
            input_feature[:, 0:5] = (input_feature[:, 0:5] - feature_mean) / feature_std * \
                                    eval_mask.unsqueeze(1).expand_as(input_feature[:, 0:5])  # dxyzi
            ...
            if mode == "Train":
                ...
                if 'feat_2d' in output_dict.keys():
                    ...
                    feat_2d = F.interpolate(feat_2d, (h, w), mode="bilinear", align_corners=True)

                # supervised loss
                total_loss = torch.tensor(0.0).cuda()

                if self.settings.loss_w_ce_2d > 0:
                    ce_loss_2d = self.criterion["focal_loss"](pred_2d, train_label, mask=wss_mask)
                ...
                if entropy_selection:
                    with torch.no_grad():
                        select_ratio = np.log(1 + (1 + epoch) / self.settings.n_epochs) / np.log(2)  # [0, 1]
                        select_ratio = select_ratio * 0.5

                        train_label_contra, wss_mask_contra = self.entropy_based_selection(output=pred_2d,
                                                                                           wss_mask=wss_mask,
                                                                                           eval_mask=eval_mask,
                ...
                if self.settings.loss_w_contrast > 0 and return_feat:
                    ...
                    contrast_loss = self.criterion['contrast_loss'](feats=feat_2d,
                                                                    output=pred_2d,
                    ...
                    total_loss = total_loss + self.settings.loss_w_contrast * contrast_loss
            ...
            with torch.no_grad():
                ...
                if mode == 'Train':
                    ...
                    if self.settings.loss_w_ce_2d > 0:
                        loss_softmax_meter.update(ce_loss_2d.item(), input_feature.size(0))
                    if self.settings.loss_w_lov_2d > 0:
                        loss_lovasz_meter.update(lov_loss_2d.item(), input_feature.size(0))
                    if self.settings.loss_w_contrast > 0 and return_feat:
                        loss_contrast_meter.update(contrast_loss.item(), input_feature.size(0))
            ...
            if self.recorder is not None:
                log_str = ">>> {} E[{:03d}|{:03d}] I[{:04d}|{:04d}] DT[{:.3f}] PT[{:.3f}] LR {:0.5f} ".format(
                    mode, self.settings.n_epochs, epoch + 1, total_iter, i + 1, data_cost_time, process_cost_time, lr)
        ...
        if self.recorder is not None:
            if mode == 'Train':
                ...
                if self.settings.loss_w_ce_2d > 0:
                    self.recorder.tensorboard.add_scalar(
                        tag="{}_LossSoftmax".format(mode), scalar_value=loss_softmax_meter.avg, global_step=epoch)
                if self.settings.loss_w_lov_2d > 0:
                    self.recorder.tensorboard.add_scalar(
                        tag="{}_LossLovasz".format(mode), scalar_value=loss_lovasz_meter.avg, global_step=epoch)
                if self.settings.loss_w_contrast > 0 and return_feat:
                    self.recorder.tensorboard.add_scalar(
                        tag="{}_LossContrast".format(mode), scalar_value=loss_contrast_meter.avg, global_step=epoch)

            self.recorder.tensorboard.add_scalar(
                tag="{}_mean_Acc_3D".format(mode), scalar_value=mean_acc3d.item(), global_step=epoch)
            self.recorder.tensorboard.add_scalar(
                tag="{}_mean_IOU_3D".format(mode), scalar_value=mean_iou3d.item(), global_step=epoch)
            ...
            for i, iou in enumerate(class_iou3d.cpu()):
                if i not in [0]:
                    self.recorder.logger.info("class {:02d} {} iou: {:3f}".format(
                        i, dataloader.dataset.mapped_cls_name[i], iou))

            # per class 3D iou 
            for i, (_, v) in enumerate(self.mapped_cls_name.items()):
                self.recorder.tensorboard.add_scalar(
                    tag="{}_IOU_{:02d}_{}".format(mode, i, v), scalar_value=class_iou3d[i].item(), global_step=epoch)
            ...
            self.recorder.tensorboard.add_image("{}_Images".format(mode), output_images, epoch)
            ...
            self.recorder.tensorboard.add_image("{}_PLImages".format(mode), output_images, epoch)
