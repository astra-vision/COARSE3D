...
class Nuscenes(data.Dataset):
    def __init__(
        ...
        print("{}: {} sample: {}".format(version, self.split, len(self.token_list)))
    ...
    def loadLabelByIndex(self, index):
        ...
        if self.split == "test":
            ...
            annotated_data = np.fromfile(lidarseg_path, dtype=np.uint8).reshape(
                (-1, 1)
            )  # label
    ...
    def loadDataByIndex(self, index, min_dist: float = 1.0):
        ...
        lidar_path = os.path.join(
            self.data_path, self.nusc.get("sample_data", lidar_sample_token)["filename"]
        )
        ...
        if self.has_weak_label:
            weak_label_path = (
                ...
                .replace("lidarseg", self.weak_label_name)
                .replace(".bin", ".npy")
            )
