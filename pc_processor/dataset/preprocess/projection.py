...
class RangeProjection(object):
    ...
    def __init__(self, fov_up=3, fov_down=-25, proj_w=512, proj_h=64, fov_left=-180, fov_right=180):
        # check params
    ...
    def doProjection(self, pointcloud: np.ndarray, depth: np.ndarray=None):
        ...
        proj_x = (yaw + abs(self.fov_left)) / self.fov_hori  # normalized in [0, 1] # evenly divide in horizon
        # proj_x = 0.5 * (yaw / np.pi + 1.0) # normalized in [0, 1]
        proj_y = 1.0 - (pitch + abs(self.fov_down)) / self.fov_vert  # normalized in [0, 1]
