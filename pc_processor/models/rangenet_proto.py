...
class RangeNetProto(nn.Module):
    def __init__(self,
        ...
        self.decoder = Decoder(layer=layers,
                               # stub_skips=stub_skips,
                               OS=32,
                               feature_depth=self.backbone.get_last_depth())
        #
        self.head = nn.Sequential(nn.Dropout2d(p=0.01 if layers == 21 else 0.05),
    ...
    def forward(self, x, label=None, eval_mask=None, return_feat=False, proto_loss=False, proto_pl=None,
        ...
        if return_feat:
            if self.proj_feat == 'mix':
                ...
                h = int(h / 2)
                w = int(w / 2)
                feat0 = F.interpolate(skips[1], size=(h, w), mode="bilinear", align_corners=True)  # 32
                feat1 = F.interpolate(skips[2], size=(h, w), mode="bilinear", align_corners=True)  # 64
                feat2 = F.interpolate(skips[4], size=(h, w), mode="bilinear", align_corners=True)  # 128
                feat3 = F.interpolate(skips[8], size=(h, w), mode="bilinear", align_corners=True)  # 256
        ...
        if self.use_prototype and label is not None and eval_mask is not None:
            ...
            out_feat = rearrange(embedding, 'b c h w -> (b h w) c')  # [1, 720, 128, 256]  => [32768, 720]
...
if __name__ == '__main__':
    ...
    model = RangeNetProto(nclasses=20,
                          proj_dim=128,
                          projection='v1',
                          proj_feat='mix',
                          l2_norm=True,
